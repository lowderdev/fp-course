...
specialCharParser =
  ...
  where
    special =
      character >>= \c ->
        optional (valueParser . fromSpecialCharacter) (unexpectedCharParser c) (toSpecialCharacter c)
...
jsonValue = spaces *> jsonParser <* spaces
  where
    jsonParser =
      (JsonNull <$ jsonNull)
        ||| (JsonTrue <$ jsonTrue)
...
-- "{\"friends\":[\"pfmrph\", \"pedrofurla\", \"countoren\"], \"jsonParserDone?\":true, \"blah\":-123.45}"
