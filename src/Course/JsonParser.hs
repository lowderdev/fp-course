...
-- | Parse a JSON string.
-- Handle double-quotes, special characters, hexadecimal characters.
...
jsonString =
  between (is '"') (charTok '"') (list jsonStringContent)

-- | Helper function for jsonString
jsonStringContent :: Parser Char
jsonStringContent =
  character >>= \case
    '\\' ->
      character >>= \case
...
-- | Helper function for jsonString
trySpecialChar :: Char -> Parser Char
trySpecialChar c =
  optional
...
jsonNumber = tok $
  P
    ( \i -> case readFloats i of
        ...
        Full (rational, i') -> Result i' rational
    )
...
jsonObject =
  betweenSepbyComma
    ...
    ( jsonString >>= \jss ->
        charTok ':'
          >> jsonValue >>= \jsv -> valueParser (jss, jsv)
    )
