...
moveLeftN' n lz@(ListZipper l _ _)
  | n == 0 = Right lz
  | n < 0 = moveRightN' (negate n) lz
  | otherwise = optional Right (Left (length l)) (toOptional (moveLeftN n lz))
...
moveRightN' n lz@(ListZipper _ _ r)
  ...
  | otherwise = optional Right (Left (length r)) (toOptional (moveRightN n lz))
...
nth n lz@(ListZipper l _ _) = moveRightN (n - length l) lz
...
instance Applicative ListZipper where
  -- /Tip:/ Use @List#repeat@.
  -- `ListZipper Nil a Nil` still passes tests
...
instance Applicative MaybeListZipper where
  ...
  (<*>) (MLZ (Full lzf)) (MLZ (Full lzx)) = isZ (lzf <*> lzx)
  (<*>) _ _ = isNotZ
...
instance Extend ListZipper where
  (<<=) f lz = ListZipper (g moveLeft) (f lz) (g moveRight)
    where
      g moveFunc = unfoldr (h moveFunc) lz
      h moveFunc = (<$>) (\z' -> (f z', z')) . toOptional . moveFunc
...
instance Traversable ListZipper where
  traverse f (ListZipper l x r) = lift3 ListZipper (g l) (f x) (g r)
    where
      g = sequenceA . (f <$>)
