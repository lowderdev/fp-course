...
instance Functor k => Functor (StateT s k) where
  ...
  (<$>) ab ska = StateT (\s -> Bf.first ab <$> runStateT ska s)
...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) skab ska =
    StateT (\s -> (\(ab, s') -> Bf.first ab <$> runStateT ska s') =<< runStateT skab s)
...
instance Monad k => Monad (StateT s k) where
  ...
  (=<<) askb ska = StateT (\s -> (\(a, s') -> runStateT (askb a) s') =<< runStateT ska s)
...
state' sas = StateT (ExactlyOne . sas)
...
execT ska s = P.snd <$> runStateT ska s
...
evalT ska s = P.fst <$> runStateT ska s
...
distinctF xs = evalT state S.empty
  where
    state = filtering f xs
    f = \x -> StateT (\s -> if x > 100 then Empty else Full (S.notMember x s, S.insert x s))
...
instance Monad k => Applicative (OptionalT k) where
  ...
  (<*>) (OptionalT ab) (OptionalT a) = OptionalT (optional (\ab' -> (ab' <$>) <$> a) (pure Empty) =<< ab)
...
instance Monad k => Monad (OptionalT k) where
  ...
  (=<<) f x = OptionalT (g =<< runOptionalT x)
    where
      g Empty = pure Empty
      g (Full y) = runOptionalT (f y)
...
instance Applicative (Logger l) where
  ...
  (<*>) (Logger ls f) (Logger ks x) = Logger (ls ++ ks) (f x)
...
distinctG xs = runOptionalT $ evalT filtered S.empty
  where
    filtered = filtering predicate xs
    predicate = \x -> StateT (OptionalT . checkOver100 x)
    checkOver100 x s =
      if x > 100
        then log1 ("aborting > 100: " ++ show' x) Empty
        else logIfEven x (Full (S.notMember x s, S.insert x s))
    logIfEven x =
      if even x
