...
-- betweenCharTok o c = tok $ between (char o) (charTok c)
...
hex =
  getHexString
    >>= lift2 (optional (pure . chr)) (constantParser . UnexpectedString) readHex
  where
    getHexString = replicateA 4 (satisfy isHexDigit)
...
-- | Write a function that produces a non-empty list of values coming off the given
-- parser (which must succeed at least once), separated by the second given parser.
--
...
-- True
-- TODO check for correct parse result `ExpectedEof`?
eof :: Parser ()
eof = P $ \i -> if i == "" then Result "" () else ExpectedEof i
